...
async function main() {
  ...
  const session1 = await db.gameSession.create({
    data: {
      ...
      description:
        'Enter the mists of Barovia and face the vampire Strahd von Zarovich. New players welcome!',
      system: 'D&D 5e',
      minPlayers: 3,
      maxPlayers: 5,
  ...
  const session2 = await db.gameSession.create({
    data: {
      ...
      description:
        'A thrilling Pathfinder one-shot in the classic dungeon format. Pre-made characters available.',
  ...
  const event1 = await db.event.create({
    data: {
      ...
      description:
        'Monthly open game night! Bring your favourite board games or just show up and join a table.',
  ...
  const event2 = await db.event.create({
    data: {
      ...
      description:
        'Teams of 4 compete in a parallel one-shot dungeon. Prizes for fastest clear and best roleplay.',
  ...
  const application1 = await db.memberApplication.create({
    data: {
      ...
      status: 'PENDING',
      message:
        "I've been attending game nights for 3 months and would love to become a full member!",
    },
  });
